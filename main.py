...
2. **混合架构浏览系统**
   - 主标签页维护列表：登录和获取主题列表使用单标签页
   - 子标签页独立浏览：每个主题在新标签页中打开，确保完整页面生命周期
   - 智能标签页管理：自动复制Cookies保持登录状态，浏览完成后及时关闭
...
3. **深度行为模拟引擎**
   ...
   - 强制已读标记：滚动到底部并停留5-8秒，确保Discourse计数
...
注意事项
--------
- 复杂验证码（reCAPTCHA v3）可能需要人工介入
- "已读统计"效果依赖Discourse站点具体实现逻辑
