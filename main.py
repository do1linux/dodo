...
class BrowserManager:
    ...
    def init_browser():
        ...
        try:
            # 方法1: 直接传递选项
            browser = ChromiumPage(chromium_options=co)
        except TypeError as e:
            # 方法2: 如果上面的方法不行，尝试其他方式
            logger.warning(f"第一种初始化方式失败: {e}, 尝试备用方法")
            try:
                # 备用方法: 先创建页面再设置选项
                browser = ChromiumPage()
                # 手动设置一些选项
                for arg in browser_args:
                    browser.set.browser_argument(arg)
                browser.set.user_agent(user_agent)
