...
def main():
    ...
    if site_selector == 'all':
        sites_to_run = ['linux_do', 'idcflare']
    else:
        sites_to_run = [site_selector]
    
    logger.info(f"🎯 选择的站点: {', '.join(sites_to_run)}")
    
    success_sites = []
    
    for site_key in sites_to_run:
        if site_key not in SITES:
            logger.error(f"❌ 未知站点: {site_key}")
            continue
        
        site_config = next(site for site in SITES if site['name'] == site_key)
        credentials = SITE_CREDENTIALS.get(site_key, {})
        
        # 检查凭证
        if not credentials.get('username') or not credentials.get('password'):
            logger.error(f"❌ 跳过 {site_key} - 缺少环境变量")
            continue
